...
def is_pos_reached(set_pos,actual_pos):
    ...
    for key in actual_pos:
        ...
        reached.append(actual_pos[key] + 0.1 >= set_pos[key] >= actual_pos[key] - 0.1)
    if all(reached):
        pos_reached = 1
    else:
        pos_reached = 0
...
def draw_label(img, detection):
    ...
    for i in range(len(detection)):
        ...
        cv.rectangle(img, pt1, pt2, (255, 0, 0), 1)
        cv.putText(img, label, pt1, cv.FONT_HERSHEY_PLAIN, 1, (0, 255, 0), 2)
    return img
...
while viewer.is_available():    #True:
    ...
    orange_mask, orange_mask_in_BGR, enhanced_LAB_into_BGR = gate_preprocessing(frame, lower_bound,
                                                                                                  upper_bound)
